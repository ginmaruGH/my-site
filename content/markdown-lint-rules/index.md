...
※複数のリストをわけて表示する場合、段落をはさむ。空白行では同じリストとみなされる。
...
基本的にMarkdownファイル最初の行は`H1`の見出しにする。
...
このサイトの記事はMarkdownで書いているが、CSSでスタイリングを加えているのでルールから外れているものがあるかもしれない。

Markdownのルールを完全に踏襲する限りではないが、スタイルの一貫性を保つためにも今後のテーマづくりに活かしたい。
